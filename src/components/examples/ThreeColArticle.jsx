export default function ThreeColArticle() {
  return (
    <div className="grid example classic-article">
      <header className="article-title">
        <h3>Lorem Ipsum Dolor</h3>
      </header>
      <aside className="sidebar-left">
        <p className="article-text">
          <b>Recent Articles</b>
        </p>
        <p className="article-text">
          <a href="#">Volutpat ac</a>
        </p>
        <p className="article-text">
          <a href="#">Migurd eneos</a>
        </p>
        <p className="article-text">
          <a href="#">Ciborgifis</a>
        </p>
        <p className="article-text">
          <a href="#">Ix Peron Al</a>
        </p>
      </aside>{" "}
      <article className="article">
        <p className="article-text">
          <i>by Ixion</i>
        </p>
        <time></time>
        <p className="article-text">
          Dolor purus non enim praesent elementum facilisis leo vel. Iaculis
          nunc sed augue lacus. Quam id leo in vitae turpis massa sed elementum.
          Sed vulputate odio ut enim blandit volutpat maecenas volutpat blandit.
          Vitae turpis massa sed elementum.
        </p>
        <p className="article-text">
          Vitae turpis massa sed elementum tempus egestas sed sed risus. Auctor
          neque vitae tempus quam pellentesque nec nam aliquam. Justo eget magna
          fermentum iaculis eu non diam. Mauris ultrices eros in cursus turpis.
          Suscipit tellus mauris a diam. Penatibus et magnis dis parturient.
          Etiam sit amet nisl purus in mollis. Mattis molestie a iaculis at
          erat. Ipsum dolor sit amet consectetur. Leo duis ut diam quam nulla
          porttitor. At urna condimentum mattis pellentesque id nibh tortor id.
          Parturient montes nascetur ridiculus mus. Arcu dictum varius duis at
          consectetur lorem donec massa sapien. Et netus et malesuada fames ac
          turpis egestas maecenas pharetra. Tristique senectus et netus et
          malesuada fames ac.
        </p>
        <p className="article-text">
          Mattis aliquam faucibus purus in massa tempor nec feugiat. Tempus quam
          pellentesque nec nam. Rutrum tellus pellentesque eu tincidunt tortor
          aliquam nulla. Viverra vitae congue eu consequat ac. Tincidunt
          lobortis feugiat vivamus at augue eget arcu. Hendrerit gravida rutrum
          quisque non tellus orci ac. In ante metus dictum at tempor commodo
          ullamcorper a. Elit duis tristique sollicitudin nibh. Sit amet tellus
          cras adipiscing enim eu turpis egestas pretium. Ipsum consequat nisl
          vel pretium lectus quam id leo in. Nisl nunc mi ipsum faucibus vitae
          aliquet nec ullamcorper sit. Ridiculus mus mauris vitae ultricies leo
          integer malesuada nunc vel. Tortor aliquam nulla facilisi cras
          fermentum odio eu feugiat. Netus et malesuada fames ac turpis egestas
          maecenas pharetra convallis. Placerat orci nulla pellentesque
          dignissim enim.
        </p>
      </article>
      <aside className="ad">
        <img src="../../examples/pexels-article-ad.jpg" alt="Tonic ad" />
        <i>Buy Tonic&#8482;!</i>
      </aside>
      <footer>
        <i>&copy; Dignissim Lectus LLC MCCCVII</i>
      </footer>
    </div>
  );
}
